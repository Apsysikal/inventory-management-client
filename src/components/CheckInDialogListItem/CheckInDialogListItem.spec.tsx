...
const baseProps: ComponentProps<typeof CheckInDialogListItem> = {
  item: baseItem,
  onItemChangedCallback: baseCallback,
};

describe("<CheckInDialogListItem />", () => {
  it("Renders the serial according to the item passed in props", async () => {
    render(<CheckInDialogListItem {...baseProps} />, { wrapper: List });
  ...
  it("Renders the checkbox, it is clickable and fires the callback with the item", async () => {
    ...
    render(
      <CheckInDialogListItem
        {...{ ...baseProps, onItemChangedCallback: cb }}
      />,
      { wrapper: List }
    );
